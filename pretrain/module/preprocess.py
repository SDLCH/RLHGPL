...
def mask_features(feats, feat_mask):
    if len(feats.size()) == 3:
        mask_feats = []
        for feat in feats:
            mask_feats.append(mask_feature(feat,feat_mask))
        if torch.cuda.is_available():
            mask_feats =  [f.cuda() for f in mask_feats]
    else:
        mask_feats = mask_feature(feats, feat_mask)
        if torch.cuda.is_available():
            mask_feats = mask_feats.cuda()
    return mask_feats
