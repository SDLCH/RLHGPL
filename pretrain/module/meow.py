...
class MEOW(nn.Module):
    ...
    def forward(self, feats, mask_feat, mask_adjs, adjs_norm, num_cluster, prompt=None):
        if self.dataset == 'acm':
            h_view_embed, h_mask_embed, h_coarse = self.aggregate_nei_acm(feats, mask_feat, prompt)
        elif self.dataset == 'dblp':
            h_view_embed, h_mask_embed, h_coarse = self.aggregate_nei_dblp(feats, mask_feat, prompt)
        elif self.dataset == 'aminer':
            h_view_embed, h_mask_embed, h_coarse = self.aggregate_nei_aminer(feats, mask_feat, prompt)
        elif self.dataset == 'freebase':
            h_view_embed, h_mask_embed, h_coarse = self.aggregate_nei_acm(feats, mask_feat, prompt)
        z_new = self.generate_loss(h_view_embed, h_mask_embed, mask_adjs, adjs_norm)

        if prompt is not None:
            z_new_norm = [F.normalize(z_temp, dim=1) for z_temp in z_new]
            z = self.att(z_new_norm)
            self.z_pos = z
    ...
    def aggregate_nei_acm(self, feats, mask_feat, prompt):
        h_tar = F.elu(self.dropout(self.fc_list[0](feats[0])))
        h_mask = F.elu(self.dropout(self.fc_list[0](mask_feat)))
        if prompt is not None:
            prompt_tensor = prompt[0]
            target_prompts = prompt_tensor[self.all_cluster_labels[0]]
            h_tar = h_tar + h_tar * target_prompts
            h_mask = h_mask + h_mask * target_prompts

        h_nei = []
        for i in range(1,len(feats)):
            h_n = F.elu(self.dropout(self.fc_list[i](feats[i])))
            if prompt is not None:
                p_n = prompt[i][self.all_cluster_labels[i]]
                h_n = h_n + h_n * p_n
            h_nei.append(h_n)
        self.h_nei = h_nei
        h_view_embed = []
    ...
    def aggregate_nei_aminer(self, feats, mask_feat, prompt):
        ...
        if prompt is not None:
            prompt_tensor = prompt[0]
            target_prompts = prompt_tensor[self.all_cluster_labels[0]]
            h_tar = [h + h * target_prompts for h in h_tar]
            h_mask = [h + h * target_prompts for h in h_mask]

        h_nei = []
        for i in range(1,len(feats)):
            h_n = F.elu(self.dropout(self.fc_list[i - 1](feats[i])))
            if prompt is not None:
                p_n = prompt[i][self.all_cluster_labels[i]]
                h_n = h_n + h_n * p_n
            h_nei.append(h_n)
    ...
    def aggregate_nei_dblp(self, feats, mask_feat, prompt):
        ...
        if prompt is not None:
            feat_a = feat_a + feat_a * prompt[0][self.all_cluster_labels[0]]
            feat_a_mask = feat_a_mask + feat_a_mask * prompt[0][self.all_cluster_labels[0]]
            feat_p = feat_p + feat_p * prompt[1][self.all_cluster_labels[1]]
            feat_t = feat_t + feat_t * prompt[2][self.all_cluster_labels[2]]
            feat_c = feat_c + feat_c * prompt[3][self.all_cluster_labels[3]]

        self.h_nei = [feat_p, feat_t, feat_c]
        # apa
    ...
    def get_all_state(self):
        return [self.z_pos.detach()] + [h.detach() for h in self.h_nei]
