...
def preprocess_features(features):
    ...
    r_inv[nonzero_indices] = np.power(rowsum[nonzero_indices], -1)
    r_inv = r_inv.flatten()
...
def load_freebase(ratio, type_num):
    ...
    nei_a = sp.load_npz(path + "nei_a.npz")
    nei_w = sp.load_npz(path + "nei_w.npz")

    transformer = SparseRandomProjection(n_components=1024, random_state=0)
    feat_m = transformer.fit_transform(sp.eye(type_num[0]))
    feat_d = transformer.fit_transform(sp.eye(type_num[1]))
    feat_a = transformer.fit_transform(sp.eye(type_num[2]))
    feat_w = transformer.fit_transform(sp.eye(type_num[3]))
    ...
    nei_a = sparse_mx_to_torch_sparse_tensor(nei_a)
    nei_w = sparse_mx_to_torch_sparse_tensor(nei_w)
    ...
    return [nei_d, nei_a, nei_w], [feat_m, feat_d, feat_a, feat_w], [mam, mdm, mwm], label, train, val, test
